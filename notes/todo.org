...
@BUG: Okay what's going on with the random number generator? we never get good real number values because we do this & 0x7FFF which means our max never approaches INT_MAX, so if we use that for getting our ratio we're always very small
