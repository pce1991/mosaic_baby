...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmndLine, int nCndShow) {
    ...
    AllocateInputManager(inputManager, &gameMem->permanentArena, 32, 8);

    {
        HMODULE *XInputHandle = &platform.XInputHandle;
        
        *XInputHandle = LoadLibrary("xinput1_4.dll");
        if (*XInputHandle == NULL) {
            *XInputHandle = LoadLibrary("xinput9_1_0.dll");
        }
        if (*XInputHandle == NULL) {
        ...
        if (*XInputHandle != NULL) {
            XInputGetState_ = (xinput_get_state *)GetProcAddress(*XInputHandle, "XInputGetState");
            XInputSetState_ = (xinput_set_state *)GetProcAddress(*XInputHandle, "XInputSetState");
        }
    }
    ...
    // @HACK
    gameMem->gamepads[0] = &inputManager->devices[2];
    gameMem->gamepads[1] = &inputManager->devices[3];
    gameMem->gamepads[2] = &inputManager->devices[4];
    gameMem->gamepads[3] = &inputManager->devices[5];
    
    gameMem->gamepadCount = 1;
    AllocateInputDevice(gameMem->gamepads[0], InputDeviceType_Gamepad, Input_GamepadDiscreteCount, Input_GamepadAnalogueCount);

    

    Keyboard = gameMem->keyboard;
    Mouse = gameMem->mouse;
    Gamepad = gameMem->gamepads[0];
    ...
    while(gameMem->running && PlatformRunning) {
        ...
        for (int i = 0; i < 4; i++) {
            WinPushGamepadEvents(inputManager, gameMem->gamepads[i]);
        }
