...
void DrawMaskedQuad(vec2 position, vec2 scale, real32 angle, vec4 color, Sprite *texture) {
    Shader *shader = &Game->maskedQuadShader;
    SetShader(shader);
    ...
    glUniform1fv(shader->uniforms[3].id, 1, &Game->time);
    glUniform4fv(shader->uniforms[4].id, 1, color.data);
